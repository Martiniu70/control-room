...
interface EcgCardProps {
    ...
    height?: number;
    unit?: string;
    color?: string; // Cor específica para o ECG
...
const EcgCard: React.FC<EcgCardProps> = ({
    title,
    data,
    color = "#82ca9d", // Cor padrão para ECG
    width,
    height,
    unit = "mV",
    onClose,
    ...
    const getDisplayPrecision = (value: number, currentUnit: string) => {
        // ECG geralmente precisa de mais casas decimais
    ...
    // O EcgCard não tem botões de controlo específicos no cabeçalho (como o EEG),
    ...
    return (
        <CardWrapper
            ...
            // Passa o array de visualizações do cardConfig para 'ecg'
